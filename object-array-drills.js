...
if (band.albums.length + 1 % 2 === 0) {
    console.log(`The number ${band.albums.length+1} is even`);
} else {
    console.log(`The number ${band.albums.length+1} is odd`)
}
...
let sireTally = 0;
for (let i = 0; i < band.albums.length; i++) {
    // console.log(band.albums[i].albumDetails.label);
    if (band.albums[i].albumDetails.label === 'Sire') {
        sireTally++;
    } 
}
console.log(`the sireTally is: ${sireTally}.`);
